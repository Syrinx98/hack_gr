...
The general TT gauge plane wave solution results in
\[
h_{\mu \nu}^{TT} = \operatorname{Re}\bigl(C_{\mu \nu}\, e^{i\,k_{\sigma}\,x^{\sigma}}\bigr).
\]
We start by considering a Cartesian coordinate system \((x, y, z)\) and
\[
k^{\mu} = (\omega, 0, k, k).
\]
...
Since gravitational waves propagate in vacuum, they travel at the speed of light, and their wave vector must be null:
\[
k_{\mu} k^{\mu} \;=\; 0.
\]
...
\paragraph{(ii) Explicit expression for \(C_{\mu \nu}\) in the original system.}
...
We now define a new coordinate system \(\,(x',\,y',\,z')\), where:
\[
  \begin{cases}
    x' = x, \\
    z' = \tfrac{1}{\sqrt{2}}(y+z), \\
  ...
  \end{cases}
\]
This rotation in the \((y,z)\)-plane makes \(\hat{z}'\propto (0,1,1)\) the direction of propagation.
In TT gauge, for a wave propagating along \(z'\), the only non-zero components in the \emph{primed} frame are
\[
C'_{x'x'} = h'_{+},
\quad
C'_{y'y'} = -\,h'_{+},
...
C'_{z'\mu'} = 0.
\]
The transformation from the new frame \((t',x',y',z')\) back to \((t,x,y,z)\) is given by the matrix
\[
  \Lambda^{\mu'}_{\;\mu}
  = \frac{\partial x^{\mu'}}{\partial x^{\mu}},
  \quad
  ...
  \Lambda^{x'}_{\;x} = 1,
  \quad
  \Lambda^{y'}_{\;y} = \tfrac{1}{\sqrt{2}},
  \quad
  \Lambda^{y'}_{\;z} = -\tfrac{1}{\sqrt{2}},
  \quad
  \Lambda^{z'}_{\;y} = \tfrac{1}{\sqrt{2}},
  \quad
  \Lambda^{z'}_{\;z} = \tfrac{1}{\sqrt{2}},
  \quad
  \text{(all other components 0)}.
\]
The \emph{amplitude tensor} in the old coordinates is then
\[
C_{\mu\nu}
\;=\;
\Lambda^{\mu'}_{\;\mu}\,\Lambda^{\nu'}_{\;\nu}\,C'_{\mu'\nu'}.
\]
Below, we list the \emph{correct} final components (omitting lengthy intermediate sums).  In particular, we stress:
...
Putting everything together in matrix form (with \(\mu,\nu\in\{t,x,y,z\}\)):
...
This final \(C_{\mu\nu}\) correctly reflects:
\begin{itemize}
  \item Transversality: \(C_{t\mu} = 0\).
  \item Tracelessness in the spatial block: \(C_{xx}+C_{yy}+C_{zz}=0\).
  \item Separation of polarizations: the off-diagonal \((x,y)\) and \((x,z)\) entries depend only on \(h'_\times\), while \((y,y)\), \((y,z)\), \((z,z)\) depend only on \(h'_+\).
\end{itemize}
